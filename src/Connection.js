...
export default class Connection {
  constructor(uri, network) {
    ...
    static CONNECTION_ATTEMPT_DELAY = parseInt(Config.CONNECTION_ATTEMPT_DELAY, 10); // The maximum amount of time to wait for the socket to open before giving up
    static CONNECTING_RETRY_ATTEMPTS = parseInt(Config.CONNECTING_RETRY_ATTEMPTS, 10);
    ...
    setUri(uri){
      Logger.info(`Uri updated in connection to ${uri}`);
      this._uris = uri.split(',');
    }
    ...
    disconnect(eventHandlers, forceClosed = true) {
      this._forcedClose = forceClosed;
      if (eventHandlers){
